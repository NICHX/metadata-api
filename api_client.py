...
class MediaRenamerClient:
    ...
    def batch_recognize(
        ...
        response = requests.post(
            f"{self.base_url}/api/v1/recognition/batch-recognize",
            json=payload
        )
        return response.json()
    ...
    def preview_rename(self, file_info: List[dict]) -> dict:
        ...
        response = requests.post(
            f"{self.base_url}/api/v1/media/preview-rename",
            json=payload
        )
        return response.json()
    ...
    def clear_cache(self) -> dict:
        """清除 API 缓存"""
        response = requests.post(f"{self.base_url}/api/v1/config/clear-cache")
        return response.json()
...
def main():
    ...
    server_url = sys.argv[1] if len(sys.argv) > 1 else BASE_URL
    ...
    if not config.get("tmdb_api_key_set"):
        ...
        print("   可通过以下方式配置:")
        print("   1. 创建 .env 文件")
        print("   2. 通过 PUT /api/v1/config 接口设置")
        print("   3. 访问 /docs 使用 Swagger UI 设置")
    ...
    if config.get("tmdb_api_key_set"):
        ...
        if result.get("success"):
            ...
            if poster_path:
                # 方式一：通过代理获取图片数据
                print(f"\n5. 获取海报（方式一：API 代理）...")
                poster_url = f"{server_url}/api/v1/media/image?path={poster_path}&size=w500"
                ...
                img_resp = requests.get(
                    f"{server_url}/api/v1/media/image-url",
            ...
            if poster_path:
                print(f"\n7. 下载海报到本地...")
                img_resp = requests.get(
                    f"{server_url}/api/v1/media/image",
                    params={"path": poster_path, "size": "w500"}
                )
