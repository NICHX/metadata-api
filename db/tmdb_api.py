...
def _fetch_tmdb_season_raw(tv_id, season, api_key, language="zh-CN"):
    """Fetch full season data (all episodes + poster) from TMDb in one call."""
    if not tv_id or tv_id == "None" or not api_key.strip():
        return None
    try:
        logging.info("TMDb获取季数据: tv=%s, season=%s, lang=%s", tv_id, season, language)
        response = _tmdb_get(
            f"https://api.themoviedb.org/3/tv/{tv_id}/season/{season}",
            params={"api_key": api_key.strip(), "language": language},
            timeout=TIMEOUT_DB_DETAIL,
        )
        response.raise_for_status()
        return response.json()
    except Exception as err:
        logging.warning(f"TMDb季数据获取失败 tv=%s s=%s: %s", tv_id, season, err)
        return None


def fetch_tmdb_season_data(tv_id, season, api_key, language="zh-CN"):
    """Cached version of _fetch_tmdb_season_raw."""
    return cached_request(
        _fetch_tmdb_season_raw,
        get_cache_key("tmdb_season_v3", f"{tv_id}_{season}_{language}"),
        tv_id,
        season,
        api_key,
        language,
...
def fetch_tmdb_episode_meta_raw(
    ...
    try:
        season_data = fetch_tmdb_season_data(tv_id, season, api_key)
        if not season_data:
            return "", "", ""
        ...
        if not ep_data:
            logging.warning("TMDb季数据中未找到剧集 tv=%s s=%s e=%s", tv_id, season, episode)
            return "", "", ""
        ...
        if _is_placeholder(name) or not (plot or "").strip():
            season_data_en = fetch_tmdb_season_data(tv_id, season, api_key, language="en-US")
            if season_data_en:
                for ep in season_data_en.get("episodes", []):
                    if ep.get("episode_number") == episode:
                        en_name = ep.get("name", "")
                        if _is_placeholder(name) and en_name and not _is_placeholder(en_name):
                            name = en_name
                        if not (plot or "").strip():
                            plot = ep.get("overview", "") or plot
                        break
...
def fetch_tmdb_season_poster_raw(tv_id, season, api_key):
    ...
    season_data = fetch_tmdb_season_data(tv_id, season, api_key)
    if season_data:
        return season_data.get("poster_path", "")
    return ""
